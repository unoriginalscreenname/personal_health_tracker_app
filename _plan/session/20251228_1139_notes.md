...
## Additional Refinements (made by user)

1. **useFocusEffect instead of useEffect** - Components now reload data when screen comes into focus, not just on mount
2. **onUpdate callback** - SupplementsCard has optional `onUpdate` prop so page can reload streak data when supplements change
3. **FoodTimeline extracted** - New `components/app/FoodTimeline.tsx` component with `date` prop
...
- Use `useFocusEffect` for data loading, not `useEffect`
